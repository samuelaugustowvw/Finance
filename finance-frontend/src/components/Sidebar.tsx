...
export function Sidebar() {
  ...
  const [categories, setCategories] = useState(() => {
    const userId = JSON.parse(localStorage.getItem('finance:user') || '{}').id || 'default'
    const saved = localStorage.getItem(`finance:categories:${userId}`)
  ...
  function addCategory() {
    ...
    const updated = [...categories, { name: newCatName.trim(), color: newCatColor }]
    setCategories(updated)
    const userId = JSON.parse(localStorage.getItem('finance:user') || '{}').id || 'default'
    localStorage.setItem(`finance:categories:${userId}`, JSON.stringify(updated))
    setNewCatName('')
    ...
    <aside className="w-52 flex-shrink-0 bg-gray-900 border-r border-gray-800 flex flex-col h-screen">
      ...
      <nav className="p-2 flex-1 overflow-y-auto">
        ...
        {navItems.map((item) => (
          item.disabled ? (
            <div key={item.path}
                className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm mb-0.5 text-gray-600 cursor-not-allowed">
        ...
        {categories.map((cat: { name: string; color: string }) => (
          <div key={cat.name} className="flex items-center gap-2 px-3 py-1.5 text-sm text-gray-400">
